...
public class GetWaether {

	public static void getWeather(WeatherBean wBean) throws IOException {
		String API_KEY = "6038c56ed60cff147d06d2b556e15e12";
		// Build the API call URL by adding city+country into a URL
		// In postman: http://api.openweathermap.org/data/2.5/weather?q=lund,sweden
		// &APPID=6038c56ed60cff147d06d2b556e15e12&mode=xml
		...
		NodeList updateList = doc.getElementsByTagName("lastupdate");

		for (int temp = 0; temp < updateList.getLength(); temp++) {
			// Save a node of the current list id
			Node node = updateList.item(temp);
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				// save it
				wBean.setLastUpdate(XMLtime);

				String clouds = wBean.getCloudsStr();

				System.out
						.println(wBean.getCityStr() + " is now a " + clouds + "  LastUpdate: " + wBean.getLastUpdate());
		...
		for (int temp = 0; temp < tempList.getLength(); temp++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				// save it
				wBean.setLastTemp(XMLtemp);

				String clouds = wBean.getCloudsStr();

				System.out.println(wBean.getCityStr() + " is now a " + clouds + "  LastUpdate: " + wBean.getLastUpdate() + "The temerature is: " + wBean.getLastTemp());
